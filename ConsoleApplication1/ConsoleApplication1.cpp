...
string findCodingStrand(string templateStrand, int iteration, int numberOfBases) {
...
int main() {
	...
	vector <future<string>> mRnaStrandsFromThreads;
	...
	for (int i = 0; i < numberOfThreads; i++) {
		...
		mRnaStrandsFromThreads.push_back(async(findMRna, dnaStrand2, startingPos, maxBase));
	...
	for (int i = 0; i < numberOfThreads; i++) {
		...
		maxBase = maxBase + basesJump;
		int startingPos = basesJump * i;
		codingStrandsFromThreads.push_back(async(findCodingStrand, dnaStrand2, startingPos, maxBase));
	...
	dnaOutput << "This is the mrna strand: ";
	for (int i = 0; i < numberOfThreads; i++)
	{
		dnaOutput << mRnaStrandsFromThreads[i].get();

	}
	dnaOutput << "\n \n";

	
	dnaOutput << "This is the coding strand: ";
	for (int i = 0; i < numberOfThreads; i++)
	{
